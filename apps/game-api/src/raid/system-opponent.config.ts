import type { HeroKey, RaidResourceType, TroopType } from '@crown-and-coin/shared';
...
export interface SystemOpponentTier {
  ...
  armyCounts: readonly [number, number, number];
...
export const SYSTEM_OPPONENT_TIERS: readonly SystemOpponentTier[] = [
  tier(1, 1, 1, resources(12_000n, 10_000n, 8_000n, 6_000n), [20, 15, 10], [
  ...
  tier(2, 2, 2, resources(18_000n, 14_000n, 12_000n, 9_000n), [20, 20, 15], [
  ...
  tier(3, 3, 3, resources(26_000n, 20_000n, 17_000n, 13_000n), [25, 20, 20], [
  ...
  tier(4, 4, 4, resources(38_000n, 29_000n, 24_000n, 18_000n), [30, 25, 20], [
  ...
  tier(5, 5, 5, resources(54_000n, 40_000n, 33_000n, 25_000n), [30, 30, 25], [
  ...
  tier(6, 6, 5, resources(75_000n, 56_000n, 46_000n, 35_000n), [35, 30, 30], [
...
export interface ConfiguredSystemOpponent extends SystemOpponentDefinition {
  ...
  formationVariant: number;
}

export const SYSTEM_OPPONENTS: readonly ConfiguredSystemOpponent[] = SYSTEM_OPPONENT_TIERS.flatMap(
  (configuredTier) => configuredTier.opponents.map((opponent, formationVariant) => ({ ...opponent, tier: configuredTier, formationVariant })),
);

export const SYSTEM_OPPONENT_EXTERNAL_IDS = SYSTEM_OPPONENTS.map((opponent) => opponent.externalId);

const TROOP_ORDERS: readonly (readonly TroopType[])[] = [
  ['INFANTRY', 'ARCHER', 'CAVALRY'],
  ['CAVALRY', 'INFANTRY', 'ARCHER'],
  ['ARCHER', 'CAVALRY', 'INFANTRY'],
];
const COMMANDER_ORDER: readonly HeroKey[] = ['KNIGHT', 'RANGER', 'MAGE'];

export function systemFormation(opponent: ConfiguredSystemOpponent): readonly {
  troopType: TroopType;
  unitCount: number;
  commanderKey: HeroKey;
  commanderLevel: number;
}[] {
  const offset = opponent.formationVariant % 3;
  const countByType: Record<TroopType, number> = {
    INFANTRY: opponent.tier.armyCounts[0], ARCHER: opponent.tier.armyCounts[1], CAVALRY: opponent.tier.armyCounts[2],
  };
  return TROOP_ORDERS[offset].map((troopType, index) => {
    const commanderIndex = (index + offset) % 3;
    return {
      troopType,
      unitCount: countByType[troopType],
      commanderKey: COMMANDER_ORDER[commanderIndex],
      commanderLevel: opponent.heroLevels[commanderIndex],
    };
  });
}
