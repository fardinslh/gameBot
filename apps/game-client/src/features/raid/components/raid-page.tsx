...
import { Coins, Crown, Shield, Swords, Trophy } from 'lucide-react';
...
import { useRaidState } from '../hooks/use-raid-state';
import { BattleScene } from './battle-scene';

interface RaidPageProps { dictionary: Dictionary; locale: Locale; onNavigate(section: GameSection): void; }
const EMPTY = { GOLD: '0', FOOD: '0', WOOD: '0', STONE: '0', GEMS: '0' } as const;

export function RaidPage({ dictionary: t, locale, onNavigate }: RaidPageProps) {
    ...
    <div className="game-viewport" lang={locale} dir={locale === 'fa' ? 'rtl' : 'ltr'}>
      <main className="raid-shell">
        ...
        <div className="game-ui-layer">
          ...
          <section className="raid-content" data-player-id={state?.player.id} data-raid-state={raid.battle ? battleFinished ? 'result' : 'battle' : raid.offer ? 'offer' : 'overview'}>
            {raid.battle ? battleFinished ? (
              <div className={`raid-result raid-result--${raid.battle.result === 'ATTACKER_WIN' ? 'victory' : 'defeat'}`}>
                ...
                <h1>{raid.battle.result === 'ATTACKER_WIN' ? t.raidUi.victory : t.raidUi.defeat}</h1>
                ...
                <button className="raid-primary" onClick={() => onNavigate('kingdom')} type="button">{t.raidUi.returnKingdom}</button>
                <button className="raid-secondary" onClick={raid.clearBattle} type="button">{t.raidUi.findAnother}</button>
              </div>
            ) : (
              <div className="raid-battle-wrap"><BattleScene battle={raid.battle} onComplete={() => { raid.finishBattle(); setBattleFinished(true); }} /><div className="raid-battle-label"><Swords size={16} /> {t.raidUi.autoBattle}</div></div>
            ) : (
              <>
                <header className="raid-titlebar"><span><Swords size={19} /></span><div><h1>{t.raidUi.title}</h1><p>{t.raidUi.subtitle}</p></div><b><Trophy size={14} /> {state?.player.trophies ?? 1000}</b></header>
