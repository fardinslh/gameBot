...
export function BattleDetail({ battle, dictionary: t, onBack }: BattleDetailProps) {
  ...
  return (
    <section className="battle-detail" data-battle-detail={battle.id}>
      ...
      <div className="battle-detail__teams">{(['attacker', 'defender'] as const).map((side) => <div key={side}>{battle.teams[side].map((hero) => <figure key={`${side}-${hero.slot}`}><img alt={t.heroNames[hero.key]} src={hero.portraitAsset} /><figcaption>{t.heroNames[hero.key]} · {t.heroUi.level}<BidiValue direction="ltr">{hero.level}</BidiValue></figcaption></figure>)}</div>)}</div>
