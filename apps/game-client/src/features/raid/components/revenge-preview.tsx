...
export function RevengePreview({ dictionary: t, pending, preview, onBack, onStart }: RevengePreviewProps) {
  ...
  return (
    <section className="revenge-preview" data-revenge-preview={preview.revengeTargetId}>
      ...
      <div className="revenge-preview__team"><strong>{t.inboxUi.currentRaidTeam}</strong><span>{preview.ownArmy.squads.map((squad) => `${t.armyUi.troopNames[squad.troopType]} × ${squad.unitCount}`).join(' · ')}</span></div>
