...
export class BaleClientPlatformAdapter implements ClientPlatformAdapter {
  readonly platform: ClientPlatformType = 'bale';
  ...
  constructor() {
    if (typeof window !== 'undefined') {
      this.bridge = window.BaleApp || window.Bale || window.Telegram?.WebApp || null;
    }
  ...
  getTheme(): ClientPlatformTheme {
    ...
    return {
      bgColor: params.bg_color || (isDark ? '#0d131f' : '#ffffff'),
      textColor: params.text_color || (isDark ? '#f8fafc' : '#0d131f'),
      accentColor: params.link_color || '#00a389', // Bale signature emerald brand color
      surfaceColor: params.secondary_bg_color || (isDark ? '#1a2332' : '#f0fdf4'),
  ...
  showBackButton(onClick: () => void): void {
    ...
    if (this.bridge?.BackButton) {
      try {
        ...
        console.warn('[Platform:bale] Failed to show BackButton:', err);
      }
    }
  }
    ...
    if (callbacks) {
      for (const cb of callbacks) {
        try {
          ...
          console.error(`[Platform:bale] Error in '${event}' callback:`, err);
        }
      }
    }
  }

  private extractBaleUser(): ClientPlatformUser | null {
    try {
      const tgUser = this.bridge?.initDataUnsafe?.user;
      if (tgUser?.id) {
        return {
          id: String(tgUser.id),
