...
- **Consequences**: Heroes render as non-interactive, grounded full-body world figures and share the 14-actor world budget. Raid start remains immediate and server-authoritative; the client adds only a bounded minimum departure beat with a small force leaving the Kingdom. Victory and defeat return presentations differ, including victory-only decorative loot, but loot, balances, casualties, and outcomes are never recalculated or written during return. Revenge and Campaign remain unchanged.
