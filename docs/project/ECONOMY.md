...
The client mirrors this rule only for its ready estimate: it aggregates production per resource, clamps each capped resource to `max(capacity - balance, 0)`, then totals the collectible amounts. `POST /kingdom/collect` remains authoritative. The main four-cell HUD presents Gold, Food, Wood, and Stone as `current / capacity`, then either the authoritative per-hour Building rate or compact `FULL` / `پر` text when at or above capacity. Gems sit outside that storage grid in a separate icon-and-balance premium pill. The HUD does not expose a separate overflow label. After Collect success, React stores final balances immediately while the HUD performs a presentation-only 900ms BigInt count-up and localized per-resource gain feedback; reduced-motion users see the final values immediately.
