...
For building evolution, open `/dev/buildings`, confirm `Theme DEFAULT`, compare every quick tier for each core building, then use N vs N+1 at representative minor upgrades including 1→2, 6→7, and 19→20. Toggle Construction and inspect 100/150/200%. Verify exact Lv. 1/8/12/20 badges in the 320/375/390 viewport-equivalent panels. In the status-overlay fixture, inspect Castle, Farm, Lumber Mill, Mine, Grand Market, Academy, Blacksmith, Watchtower, and Workshop in normal, upgrade, active, and selected states; its `data-status-overlap` value must remain `false`. Owner judgment—not passing automation—approves art quality.
