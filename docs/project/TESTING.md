...
| `npm run audit:kingdom-render-quality` | Terrain, all active tier-5 buildings, three Heroes, Guard, Worker, Merchant, Scholar, and Builder source-to-physical-pixel budgets at DPR 2 and 100/125/150/200% inspection |
...
| `npm run validate:raid` | New-player shield/system offer, Army Match Offer and changed-Army rejection/fresh-search recovery, immediate training HUD balances, authoritative three-figure departure, deterministic victory/defeat, three-lane Pixi playback, distinct world-figure Kingdom returns with victory-only loot cart, no duplicate settlement, six Army snapshots/events, HUD refresh, mobile locales |
...
| `npm run validate:kingdom-soul` | Unit coverage including atlas slicing, named Pixi AnimatedSprite states/frame advancement, sprite-based inhabitants, cold-load and failed-load reduced-motion return completion, and victory-only return loot, plus identity defaults/update/persistence, Castle milestones through level 20, building activity/construction, active-Formation grounded Hero homes/assets, combined 14-actor cap, hidden-page pause, reduced motion, Persian/English 320/375/390, navigation, overflow, screenshots, and browser console |
| `npm run validate:kingdom-render-quality` | CSS canvas/logical renderer/physical framebuffer/effective resolution/browser DPR matrix at DPR-equivalent 1/1.25/1.5/2/3 plus 390x844 to 320x568 resize and clean browser console |
