...
Phase 08.2B adds a separate non-interactive Hero-presence layer. `GET /army` remains the source: only Commanders in the active Formation appear, with a grounded Knight near Castle, patrolling Ranger near Lumber Mill, and staff-glow Mage near Academy after its real server unlock. Up to three transparent world-scale Hero figures reduce the ambient allocation so the combined visible world population remains at most 14. They have no badge, label, collision, pathfinding, gameplay state, or pointer events and follow the same hidden-page and reduced-motion rules.
