...
The compact Kingdom header gives the game title primary emphasis, keeps the localized ruler title secondary, and presents Kingdom Level as a crest-attached numeric chip. A separate centered premium-currency pill inside the player profile contains only the unchanged Gem icon and uncapped Gem balance. The main Resource HUD is a 62px four-column grid for Gold, Food, Wood, and Stone. Each equal-width cell gives its unchanged icon, primary `current / capacity`, and server-derived `+rate/h` or `+rate/س` separate vertical tracks with measured space between their rendered bounds; at or above capacity the third track becomes compact `FULL` / `پر`. Adjacent Kingdom controls and Army/Raid scroll surfaces begin below the taller bar. Internal `data-capacity-state` still distinguishes normal/full/overflow for behavior and validation, but no separate overflow wording or Live badge appears. `CollectControl` uses BigInt per-resource remaining room before aggregating its estimate and distinguishes genuine zero accrual from storage-blocked production. A successful Collect applies authoritative state immediately, then animates only displayed balances with an ease-out count-up and transient positive per-resource gains; refreshes and non-Collect mutations cancel the presentation layer, while reduced motion snaps to the final values.
