...
For a standard Raid, the active authoritative Army preview now appears as three full-body Commanders and troop silhouettes leaving a Kingdom origin while `POST /raid/start` is already in flight. The normal minimum beat is 1,050 ms and reduced-motion mode uses 180 ms; neither delays the request itself nor changes the response. After stored replay and result presentation, Return to Kingdom derives a one-shot grounded Commander force from the replay. Victory carries a decorative loot cart and authoritative settled loot context; defeat returns without loot and at subdued opacity. This return creates no server call or settlement and clears after playback. In reduced-motion mode, the Hero return renderer waits for world-figure assets, mounts the final static state, and owns its near-immediate completion timer; it does not depend on the disabled Pixi ticker or an external scene timeout. Revenge and Campaign retain their existing navigation flows.
